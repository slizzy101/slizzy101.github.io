...
let URL = "https://teachablemachine.withgoogle.com/models/7sfjkp4S_/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 10 && time <= 13.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 16 && time <= 19.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if ((time >= 22 && time <= 25.5 && !poseState.firstWindowTriggered) ||
                    (time >= 22 && time <= 25.5 && !poseState.secondWindowTriggered)) {
                    if (time <= 13.0) {
                ...
                break;
            case '4':
                if (time >= 29 && time <= 33.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 33.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/7sfjkp4S_/");
    }
}
